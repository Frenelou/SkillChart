...
export default {
  ...
  mounted() {
    const height = 1000;//+svg.attr("height");
    const width = 1000;//+svg.attr("width");
    ...
    const data = this.makeGroups(this.skills);
    ...
    function newdata(animate = true) {
      ...
      // TODO: Change link color based on type
      let links = linkgroup
        .selectAll("path")
      ...
      newnodes.append("text")
        .attr("dy", "0.31em")
        .text(d => {
          return d.data.data.label || d.data.data.name;
        })
        .clone(true).lower()
        .attr("stroke", "white");
